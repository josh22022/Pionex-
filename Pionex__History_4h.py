"""
Case:Pionex__History_4h
Description:
    獲取起始時間至今每4小時的幣價，存為CSV，可透過EXCEL可進行分析。建議直接以Python執行。
How to use:
...
for i in range(1,len(currency_dict)+1):
    ...
    df = pd.DataFrame(requests.get('https://www.pionex.com/kline/query_unite_candle_data?base='+currency_dict[str(i)]+'&quote=USDT&market=pionex.v2&start='+str(ts_start)+'&end='+str(ts_end)+'&interval=4h&from=web').json())  
    j = (ts_end - ts_start)/(60*60*4)
